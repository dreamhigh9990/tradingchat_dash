...
export function ImageModal({ imageData, closeModal }) {
  ...
  return (
    <div className="fixed inset-0 z-40">
      <motion.div initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className='fixed h-screen w-screen bg-neutral-900/60'
        onClick={closeModal}
      />
              ...
              <picture className='group relative flex max-w-3xl'>
                ...
                <a
                  className='trim-alt absolute bottom-0 right-0 mx-2 mb-2 translate-y-4
                             rounded-lg bg-neutral-900/40 px-2 py-1 text-sm text-primary/80 opacity-0 transition
