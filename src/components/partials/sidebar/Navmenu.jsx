...
const Navmenu = ({ menus }) => {
  ...
  const { unreadMsgs, isRoom, selectedChat } = useChatContext();
            ...
            {!item.child && !item.isHeadr && (
              (item.title != "Logout") ? (
                <Link className="menu-link" href={item.link}>
                  ...
                  {item.link == LINKS.CHAT.ROUTE && unReadPersonalMsgCount > 0 && <span className="menu-badge">{unReadPersonalMsgCount}</span>}
                </Link>
              ) : (
